...
# Добавьте вызовы функций add_roi() , затем add_price_per_minute() .
# Обеим передайте как параметр исходную таблицу oscar_data.
# Потом рассчитайте по каждому жанру средние величины:
# рейтинг, длину, ROI и бюджет минуты.
# Для этого воспользуйтесь уже готовой функцией column_mean()
# Сведите информацию в такую таблицу:
# Жанр      | Рейтинг | Длина  | ROI   | Бюджет за минуту
# -------------------------------------------------------
# история   |     ... |    ... |   ... |              ...
# ...
# Сохраните её в переменной genres_means и выведите на экран.
# Числа печатайте с двумя знаками после запятой и выравнивайте по правому краю, а названия — по левому.
...
for genre in selected_genres:
    ...
    mean_score = column_mean(filtered_data, 2)
        
    # средняя длина (индекс столбца 3)
    mean_length = column_mean(filtered_data, 3)

    # среднее значение ROI (индекс столбца 7)
    mean_roi = column_mean(filtered_data, 7)

    # средняя цена за минуту (индекс столбца 8)
    mean_ppm = column_mean(filtered_data, 8)
